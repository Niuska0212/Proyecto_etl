...
def clean_data(df):
    """
    Limpia el DataFrame eliminando valores nulos y corrigiendo inconsistencias.
    """
...
def transform_data(df):
    """
    Transforma el DataFrame añadiendo nuevas columnas o modificando las existentes.
    """
    # Crear una nueva columna que sume las noches de fin de semana y entre semana
    df['total_nights'] = df['stays_in_weekend_nights'] + df['stays_in_week_nights']

    # Convertir la columna 'arrival_date_year' a tipo datetime si es necesario
...
def transform_data_callback(n_clicks, _):
    ...
    try:
        ...
        if df_cleaned is None:
            return html.Div("Error en la limpieza de datos", style={'color': 'red'})
            
        df_transformed = transform_data(df_cleaned)
        
        if df_transformed is None:
            return html.Div("Error en la transformación de datos", style={'color': 'red'})
        ...
        return html.Div(f"Error inesperado: {str(e)}", style={'color': 'red'})
...
def train_and_evaluate_model(n_clicks):
    ...
    try:
        ...
        if 'arrival_date' in X.columns:
            X = X.drop(columns=['arrival_date'])  # Eliminar columnas datetime
